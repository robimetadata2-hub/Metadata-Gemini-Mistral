...
export const UploadPanel: React.FC<UploadPanelProps> = (props) => {
    const { user, onLogin, stagedFiles, setStagedFiles, progress, isGenerating, isPaused, setIsPaused, startGeneration, generatedMetadata, setProgress, showToast, selectedStockSite, onStockSiteChange, fileExtension, clearAll, onExportCsv } = props;
  ...
  return (
    <div className="panel-bg p-4 sm:p-6 bg-[#202123] border border-[#363636] rounded-xl shadow-lg">
      ...
      {user ? (
        ...
        <div className="w-full h-64 border-2 border-dashed border-gray-700 rounded-lg flex flex-col items-center justify-center bg-[#252629] text-gray-400 gap-4">
            <i className="bi bi-lock-fill text-4xl text-gray-500"></i>
            <div className="text-center">
                <p className="text-lg font-medium text-gray-300">Login Required</p>
                <p className="text-sm text-gray-500">Please login to upload and process files.</p>
            </div>
            <button 
                onClick={onLogin} 
                className="px-6 py-2 bg-[color:var(--theme-color)] hover:bg-[color:var(--theme-color-hover)] text-white rounded-lg font-medium transition-colors"
            >
                Login Now
            </button>
        </div>
