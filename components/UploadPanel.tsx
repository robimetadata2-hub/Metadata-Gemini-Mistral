...
import { generateThumbnail } from '../services/fileProcessor';
...
const UploadPrompt: FC = () => (
    <div className="col-span-3 sm:col-span-4 md:col-span-6 lg:col-span-8 xl:col-span-8 flex flex-col items-center justify-center p-10 text-center min-h-[300px]">
...
const GalleryItem: FC<{ fileState: StagedFile, onRemove: (id: string) => void }> = ({ fileState, onRemove }) => {
    ...
    return (
        <div className="gallery-item group relative w-full aspect-square rounded-lg overflow-hidden bg-[#3b3b3e] shadow-md checkered-bg">
            <img src={fileState.thumbnailDataUrl} alt={fileState.file.name} className="w-full h-full object-contain" />
            {overlay}
...
export const UploadPanel: React.FC<UploadPanelProps> = (props) => {
    const { stagedFiles, setStagedFiles, progress, isGenerating, isPaused, setIsPaused, startGeneration, generatedMetadata, setProgress, showToast, selectedStockSite, onStockSiteChange, fileExtension, clearAll, onExportCsv, onHistoryClick } = props;
    
    const [isDragging, setIsDragging] = useState(false);
    const fileInputRef = useRef<HTMLInputElement>(null);
    const readyFilesCount = stagedFiles.filter(f => f.status === 'ready').length;
    // Disable generation if generating or no files
    ...
    const remainingCount = stagedFiles.length - (GALLERY_DISPLAY_LIMIT - 1);
    
    // Calculate empty slots to keep the grid full (32 items)
    const emptySlotsCount = showOverflow ? 0 : Math.max(0, GALLERY_DISPLAY_LIMIT - stagedFiles.length);
    ...
    const handleFiles = useCallback(async (files: FileList) => {
        ...
        // 3. Process each new file sequentially for thumbnails ONLY.
        // Heavy API payload generation is now deferred to the 'startGeneration' loop (JIT processing).
        ...
        for (let i = 0; i < newFilesToStage.length; i++) {
            ...
            try {
                ...
                // Mark as ready immediately after thumbnail, apiData stays null until generation
                setStagedFiles(prev => prev.map(f => f.id === fileState.id ? { ...f, thumbnailDataUrl: thumbnailUrl, status: 'ready' } : f));
  ...
  return (
    <div className="panel-bg p-4 sm:p-6 bg-[#202123] border border-[#363636] rounded-xl shadow-lg relative">
      ...
      <div 
          ...
          onDrop={onDrop}
          onClick={onAreaClick}
          className={`relative w-full border-2 border-dashed rounded-lg transition-colors overflow-hidden cursor-pointer bg-gray-800/20 ${isDragging ? 'border-[color:var(--theme-color)] bg-[color:var(--theme-color-active-bg)]' : 'border-gray-600 hover:bg-gray-700/30'}`}
      >
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-8 gap-2 justify-center items-center p-4 min-h-[160px]">
          {stagedFiles.length === 0 ? (
                  ...
                  {showOverflow && (
                      <div className="relative w-full aspect-square rounded-lg overflow-hidden checkered-bg shadow-md flex items-center justify-center text-white font-bold text-xl text-center p-2" title={`${remainingCount} more files`}>
                      <img 
                          src={stagedFiles[GALLERY_DISPLAY_LIMIT - 1].thumbnailDataUrl}
                          alt="More files background" 
                          className="absolute inset-0 w-full h-full object-contain filter blur-sm brightness-50"
                      />
                      <span className="relative z-10">+{remainingCount}</span>
                      </div>
                  )}

                  {/* Render empty placeholder slots to maintain grid structure */}
                  {Array.from({ length: emptySlotsCount }).map((_, i) => (
                        <div key={`empty-${i}`} className="w-full aspect-square rounded-lg bg-[#2a2b2e]/30 border-2 border-dashed border-[#363636]"></div>
                  ))}
                  </>
              )}
          </div>
      ...
      {(progress.percent > 0 || isGenerating) && (
        <div className="mt-6">
            <div className="flex justify-between text-sm mb-1.5">
                ...
                <span className="text-[color:var(--theme-color)] font-bold">{Math.round(progress.percent)}%</span>
            </div>
            <div className="w-full bg-gray-700 rounded-full h-2.5 overflow-hidden">
                <div 
                    className="bg-[color:var(--theme-color)] h-2.5 rounded-full transition-all duration-300 ease-out progress-bar-animated relative" 
      ...
      <div className="mt-6 grid grid-cols-2 md:grid-cols-5 gap-3 sm:gap-4">
        ...
        <button onClick={startGeneration} disabled={isGenerateDisabled} className="col-span-2 md:col-span-1 flex items-center justify-center space-x-2 py-2.5 px-4 sm:px-5 bg-[color:var(--theme-color)] hover:bg-[color:var(--theme-color-hover)] text-white font-medium rounded-lg shadow-md transition-colors text-sm sm:text-base disabled:opacity-50 disabled:cursor-not-allowed">
            {isGenerating ? (
                <>
                    <i className="bi bi-stars animate-pulse"></i>
                    <div className="flex items-center">
                        <span className="truncate">Generating</span>
                        <div className="loading-dots ml-2 flex items-center space-x-1">
                            <span></span>
                            <span></span>
                            <span></span>
                        </div>
                ...
                <>
                    ...
                    <span className="truncate">{`Generate (${readyFilesCount})`}</span>
                </>
            )}
        </button>
        <button onClick={onExportCsv} disabled={isExportDisabled} className="flex items-center justify-center space-x-2 py-2.5 px-4 sm:px-5 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg shadow-md transition-colors text-sm sm:text-base disabled:opacity-50 disabled:cursor-not-allowed"><i className="bi bi-download"></i><span className="truncate">Export CSV</span></button>
        <button onClick={onHistoryClick} className="flex items-center justify-center space-x-2 py-2.5 px-4 sm:px-5 bg-gray-600 hover:bg-gray-700 text-white font-medium rounded-lg shadow-md transition-colors text-sm sm:text-base"><i className="bi bi-clock-history"></i><span className="truncate">History</span></button>
      </div>
    </div>
  );
};
