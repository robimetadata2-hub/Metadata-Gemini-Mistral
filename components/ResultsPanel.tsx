...
export const ResultsPanel: React.FC<ResultsPanelProps> = ({ metadata, setMetadata, showToast }) => {
        ...
        <div className="space-y-3 max-h-[600px] overflow-y-auto pr-2">
          {[...metadata].reverse().map((item, index) => (
            <div key={`${item.filename}-${index}`} className="group flex flex-col sm:flex-row items-start gap-4 p-3 bg-[#2a2b2e] rounded-lg transition-colors hover:bg-[#333437]">
              <img src={item.thumbnailUrl} alt={item.filename} className="w-24 h-24 sm:w-20 sm:h-20 object-contain rounded-md flex-shrink-0 bg-[#3b3b3e]" />
