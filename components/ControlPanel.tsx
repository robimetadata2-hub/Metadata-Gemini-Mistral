    ...
    <div className="panel-bg p-4 sm:p-6 lg:sticky top-[88px] bg-[#202123] border border-[#363636] rounded-xl shadow-lg lg:max-h-[calc(100vh-104px)] lg:overflow-y-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <button 
            ...
            className="flex items-center justify-center gap-2 py-1.5 px-3 bg-gray-700 hover:bg-gray-600 text-white font-medium rounded-lg shadow-sm transition-colors text-xs border border-gray-600"
      ...
      {/* General Settings - Batch Size & RPM */}
      <div className="mb-6 pb-6 border-b border-gray-700 space-y-5">
        <div>
            <div className="flex justify-between items-center mb-2">
                <label htmlFor="batch-size" className="text-sm font-medium text-gray-300 flex items-center gap-2">
                    Batch Size (Concurrent)
                    <div className="group relative">
                        <i className="bi bi-info-circle-fill text-gray-500 text-xs"></i>
                        <span className="tooltip invisible group-hover:visible absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-[#17181a] text-white text-xs rounded-md p-2 border border-gray-600 shadow-lg z-10 text-center">
                            Number of images to process at the same time.
                        </span>
                    </div>
                </label>
                <span className="text-sm font-semibold text-white bg-gray-700 px-2 py-0.5 rounded-md">{settings.batchSize}x</span>
            </div>
            <CustomSlider id="batch-size" min={1} max={10} value={settings.batchSize} onChange={(e) => handleFieldChange('batchSize', parseInt(e.target.value))} themeColor={themeColor} />
        </div>

        <div>
            <div className="flex justify-between items-center mb-2">
                <div className="flex items-center gap-3">
                    <label htmlFor="rpm-slider" className="text-sm font-medium text-gray-300 flex items-center gap-2">
                        Requests Per Minute (RPM)
                        <div className="group relative">
                            <i className="bi bi-info-circle-fill text-gray-500 text-xs"></i>
                            <span className="tooltip invisible group-hover:visible absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 bg-[#17181a] text-white text-xs rounded-md p-2 border border-gray-600 shadow-lg z-10 text-center">
                                Limit the number of API calls per minute. Toggle to enable/disable.
                            </span>
                        </div>
                    </label>
                    <div className="switch relative inline-block w-10 h-6 flex-shrink-0">
                        <input 
                            type="checkbox" 
                            id="rpm-toggle" 
                            checked={settings.isRpmEnabled} 
                            onChange={(e) => handleFieldChange('isRpmEnabled', e.target.checked)} 
                        />
                        <label htmlFor="rpm-toggle" className="slider cursor-pointer"></label>
                    </div>
                </div>
                <span className={`text-sm font-semibold text-white bg-gray-700 px-2 py-0.5 rounded-md ${!settings.isRpmEnabled ? 'opacity-40' : ''}`}>{settings.requestsPerMinute} / min</span>
            </div>
            <div className={!settings.isRpmEnabled ? 'opacity-40 pointer-events-none grayscale' : ''}>
                <CustomSlider id="rpm-slider" min={1} max={60} value={settings.requestsPerMinute} onChange={(e) => handleFieldChange('requestsPerMinute', parseInt(e.target.value))} themeColor={themeColor} />
            </div>
        </div>
      </div>
      ...
      <div className={`${settings.activeTab === 'metadata' ? 'block' : 'hidden'} space-y-6`}>
        <div className="pt-2 pb-2 border-b border-gray-700">
             <CustomSwitch 
                id="auto-download-csv" 
                label="Auto Download CSV" 
                checked={settings.autoDownloadCsv} 
                onChange={(e) => handleFieldChange('autoDownloadCsv', e.target.checked)} 
                tooltip="Automatically download the CSV file when all files have been processed."
            />
        </div>
      ...
      <div className={`${settings.activeTab === 'prompt' ? 'block' : 'hidden'} space-y-6`}>
        <div>
         ...
         <div className="pt-2 pb-2 border-b border-gray-700">
             <CustomSwitch 
                id="auto-download-csv-prompt" 
                label="Auto Download CSV" 
                checked={settings.autoDownloadCsv} 
                onChange={(e) => handleFieldChange('autoDownloadCsv', e.target.checked)} 
                tooltip="Automatically download the CSV file when all prompts have been generated."
            />
        </div>
      ...
      <div className="mt-8 space-y-3">
        <button onClick={onSave} className="w-full py-2.5 px-5 bg-[color:var(--theme-color)] hover:bg-[color:var(--theme-color-hover)] text-white font-medium rounded-lg shadow-md transition-colors text-sm">Save Settings</button>
      </div>
    </div>
  );
};
