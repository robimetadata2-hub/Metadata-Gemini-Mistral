...
interface ApiKeyModalProps {
  ...
  // General Props
  selectedProvider: 'gemini' | 'grok' | 'mistral' | 'groq';
  setSelectedProvider: React.Dispatch<React.SetStateAction<'gemini' | 'grok' | 'mistral' | 'groq'>>;
...
export const ApiKeyModal: React.FC<ApiKeyModalProps> = ({ 
  ...
  const handleProviderChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
      const newProvider = e.target.value as 'gemini' | 'grok' | 'mistral' | 'groq';
      ...
      if (newProvider === 'gemini') {
          ...
          setLocalSelectedModel('llama-3.2-11b-vision-preview');
  ...
  const handleModelChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      if (selectedProvider === 'groq') storageKey = 'groqModel';
  ...
  if (selectedProvider === 'gemini') {
      ...
      currentApiKeys = groqCloudApiKeys;
      setCurrentApiKeys = setGroqCloudApiKeys;
      storageKey = 'groqCloudApiKeys';
      ...
      <div className="w-full max-w-md bg-[#202123] rounded-xl border border-gray-700/80 shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 border-t border-gray-700/80">
            ...
            {selectedProvider === 'groq' && (
                <a href="https://console.groq.com/keys" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-full p-3 bg-orange-600 hover:bg-orange-700 rounded-lg font-semibold text-white transition-colors">
                    <i className="bi bi-key-fill mr-2.5"></i>Get Groq API Key
                </a>
            )}
        </div>
      </div>
