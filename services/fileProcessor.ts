...
const rasterizeSvg = async (file: File, width: number): Promise<string> => {
    ...
    return compressImage(new File([blob], file.name, {type: "image/svg+xml"}), width, 0.6);
};
...
export const processFileForApi = async (fileState: StagedFile): Promise<{ base64Data: string; mimeType: string; }> => {
    ...
    if (/^image\/(jpeg|png|gif)$/.test(file.type)) {
        // Lower quality slightly to 0.6 for faster upload
        dataUrl = await compressImage(file, maxWidth, 0.6);
